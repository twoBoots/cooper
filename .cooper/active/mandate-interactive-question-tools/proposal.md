...
2. **File Manipulation**: Agents frequently use shell commands with heredocs, pipes, and stream redirections (`cat << 'EOF' > ...`, `echo "..." > ...`) instead of dedicated native file manipulation tools (`write_to_file`, `replace_file_content`, `view_file`), bypassing IDE safety checks, permissions, and diff integrations.
...
- **Mandate Native File Tools**: Explicitly prohibit shell pipes, heredocs, and shell stream redirection for creating or editing files. Mandate the use of native file tools (`write_to_file`, `replace_file_content`, `view_file`) whenever available.
