...
### Track 2: Upstream Reconciliation & CLI Self-Update Engine (`track-cooper-updater-diff3`)
- **Scope**:
  ...
  - Implement CLI binary self-update (`cooper self-update` / `cooper update --self`) replacing executable in-place with macOS codesigning.
  - Implement 3-way diff and merge engine (`base` vs `local_custom` vs `upstream_release`) for project template synchronization (`cooper update`).
...
- **Scope**:
  ...
  - Implement agent-guided update tools: `cooper_check_updates`, `cooper_diff_updates`, `cooper_apply_update`, `cooper_self_update`.
...
### Track 4: Installer Integration & GitHub Actions Release Automation (`track-cooper-installer-packaging`)
- **Scope**:
  - Update `install.sh` with Battery's 3-tier registration lifecycle (Local compile -> Release download -> Zero-binary fallback).
  - Implement GitHub Actions CI workflow (`.github/workflows/ci.yml`) for `gofmt`, `go vet`, and race-detector test coverage.
  - Implement GitHub Actions Matrix Release workflow (`.github/workflows/release.yml`) for cross-compiling `linux-x86_64`, `linux-aarch64`, `darwin-x86_64`, `darwin-aarch64`, and `windows-x86_64.exe` on push to `main` and version tags (`v*`).
